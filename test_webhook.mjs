...
async function main() {
  ...
  const { data: taskData, error: taskError } = await supabase
    ...
    .insert({
      ...
      status: 'pending',
      photo_url: 'https://images.unsplash.com/photo-1618477388954-7852f32655ec?auto=format&fit=crop&q=80&w=400',
    })
    .select();
  ...
  const { data: initialProgram } = await supabase.from('csr_programs').select('*').order('created_at', { ascending: true }).limit(1);
  console.log('   Initial CSR Program state:', initialProgram[0]);

  // 3. Trigger API Webhook
  console.log('3. Triggering webhook at http://localhost:3000/api/trigger-automation...');
  try {
  ...
  const { data: updatedTask } = await supabase.from('tasks').select('*').eq('id', taskData[0].id).single();
  ...
  const { data: updatedProgram } = await supabase.from('csr_programs').select('*').order('created_at', { ascending: true }).limit(1);

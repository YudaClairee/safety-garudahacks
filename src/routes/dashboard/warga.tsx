...
export const Route = createFileRoute('/dashboard/warga')({
  loader: async () => {
    ...
    const { data: tasks } = await supabase
      .from('tasks')
      .select(
        'id, type, status, created_at, photo_url, company_name, location, description, reward_type, reward_value, latitude, longitude, captured_at',
      )
    ...
    const { data: activeProgramsData } = await supabase
      .from('csr_programs')
      .select(
        'id, company_name, focus_category, budget_rupiah, location, reward_type, reward_value, start_date, end_date',
      )
      .gt('budget_rupiah', 0)
      .order('created_at', { ascending: false })
    ...
    const { data: redemptionsData } = await supabase
      ...
      .select(
        'id, reward_name, reward_type, reward_value, points_cost, voucher_code, status, created_at',
      )
      .eq('user_id', session.user.id)
      .order('created_at', { ascending: false })
    ...
    function findProgramForTask(task: any) {
      if (task.company_name) {
        const byCompany = activePrograms.find(
          (program) => program.company_name === task.company_name,
        )
        if (byCompany) return byCompany
      }

      return activePrograms.find(
        (program) => program.focus_category === task.type,
      )
    }
    ...
    return {
      ...
      history: (tasks || []).map((t) => {
        ...
        const rewardType =
          t.reward_type || matchedProgram?.reward_type || 'Menunggu program CSR'
        const rewardValue = Number(
          t.reward_value ?? matchedProgram?.reward_value ?? 0,
        )
        ...
        return {
          ...
          capturedAt: t.captured_at
            ? new Date(t.captured_at).toLocaleString('id-ID', {
                day: '2-digit',
                month: 'short',
                year: 'numeric',
                hour: '2-digit',
...
const rewards = [
  {
    ...
    description:
      'Dukungan kebutuhan pangan pokok untuk rumah tangga yang membutuhkan.',
  },
  {
    id: '2',
    ...
    description:
      'Bantuan tagihan dasar dan perlindungan kesehatan agar beban darurat lebih ringan.',
  },
  {
    id: '3',
    ...
    description:
      'Pencairan fleksibel untuk kebutuhan mendesak via dompet digital pilihan Anda.',
  },
]
...
function WargaRoute() {
  ...
  const gpsTasks = initialHistory.filter(
  ...
  async function startCameraAndLocation() {
    ...
    try {
      ...
      setCameraError(
        'Gagal mengakses kamera. Pastikan memberikan izin kamera di browser Anda.',
      )
  ...
  async function handleRegisterProgram(programId: string) {
    ...
    try {
      ...
      alert(
        'Berhasil mendaftar program CSR! Anda sekarang terdaftar untuk aksi ini.',
      )
      router.invalidate()
    } catch (err: any) {
  ...
  const isRegisteredForSelected = isGeneral
    ? true
    : registrations.includes(selectedProgramId)
  const selectedProgramHasStarted = isGeneral
    ? true
    : selectedProgram?.start_date
      ? new Date() >= new Date(selectedProgram.start_date)
      : true
  const selectedProgramHasEnded = isGeneral
    ? false
  ...
  const selectedProgramCost = selectedProgram
    ? Math.round(Number(selectedProgram.reward_value || 0) * 1.12)
    : 0
  const selectedProgramHasQuota = isGeneral
    ? true
    : selectedProgram
      ? Number(selectedProgram.budget_rupiah) >= selectedProgramCost
      : false

  const isFormBlocked =
    !isGeneral &&
    (!isRegisteredForSelected ||
      !selectedProgramHasStarted ||
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (selectedProgramId !== 'general') {
      ...
      if (prog) {
        ...
        if (!isReg) {
          setError(
            'Anda belum terdaftar untuk program ini. Silakan mendaftar terlebih dahulu.',
          )
          return
        }

        const isBefore = prog.start_date
          ? new Date() < new Date(prog.start_date)
          : false
        if (isBefore) {
          setError(
            `Aksi program ini belum dimulai. Baru dimulai pada ${new Date(prog.start_date).toLocaleString('id-ID')}`,
          )
          return
        }
                ...
                {redemptions.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-slate-200 px-6 py-10 text-center">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      Klaim benefit pertamamu dari modal Tukar Safety Net.
                    </p>
                  </div>
                ) : (
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="general">
                      Cocokkan Otomatis (Pooled Funding)
                    </option>
                    {activePrograms.map((prog) => {
                      const cost = Math.round(
                        Number(prog.reward_value || 0) * 1.12,
                      )
                      const remaining =
                        cost > 0
                          ? Math.floor(Number(prog.budget_rupiah) / cost)
                          : 0
                      return (
                        <option
                          key={prog.id}
                ...
                {isFormBlocked ? (
                  <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-6 flex flex-col items-center text-center gap-3">
                    ...
                    {!isRegisteredForSelected && !selectedProgramHasEnded && (
                      ...
                      >
                        {registeringId === selectedProgramId
                          ? 'Mendaftar...'
                          : 'Daftar Aksi Sekarang'}
                      </Button>
                    )}
                  </div>
                ) : (
                              ...
                              <div className="flex flex-col gap-1 mt-1.5 text-xs text-slate-655 font-medium">
                                {latitude !== null && longitude !== null ? (
                                  ...
                                  <p className="flex items-center gap-1.5 text-amber-600">
                                    ...
                                    <span>
                                      Lokasi GPS tidak terdeteksi (Fallback
                                      Lokasi Manual)
                                    </span>
                                  </p>
                                )}
              ...
              <section className="rounded-[2rem] border border-slate-200 bg-white/95 p-6 shadow-sm space-y-4">
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-slate-500 px-1">
                  <span className="flex items-center gap-1.5">
                    ...
                    <strong>
                      {gpsTasks.filter((t) => t.status === 'Approved').length}
                    </strong>{' '}
                    Disetujui
                  </span>
                  <span className="flex items-center gap-1.5">
                ...
                {initialHistory.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-slate-200 rounded-3xl">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Unggah aksi pertamamu menggunakan formulir di atas!
                    </p>
                  </div>
                ) : (
              ...
              <div className="mt-6 space-y-4">
                {activePrograms.length === 0 ? (
                  ...
                  activePrograms.map((program) => {
                    ...
                    const startDateObj = program.start_date
                    ...
                    const isBeforeStart = startDateObj
                      ? startDateObj > now
                      : false
                    ...
                    const remaining =
                      cost > 0
                        ? Math.floor(Number(program.budget_rupiah) / cost)
                        : 0
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-xs border-t border-slate-100/50 pt-2">
                          ...
                          <span
                            className={`font-semibold px-2 py-0.5 rounded-md ${statusStyle}`}
                          >
                            {statusText}
                          </span>
                        </div>
